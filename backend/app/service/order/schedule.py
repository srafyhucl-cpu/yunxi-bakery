...
class OrderScheduleService:
    ...
    def _validate_expect_time(self, value: str, business_hours: BusinessHours) -> None:
        ...
        current_time = _normalize_beijing_time(self._now_provider())
        if expect_time < current_time.replace(tzinfo=None):
            raise ValueError("预约时间不能早于当前时间")
        if (
            expect_time.date() == current_time.date()
            and current_time.time() >= SAME_DAY_ORDER_CUTOFF
        ):
