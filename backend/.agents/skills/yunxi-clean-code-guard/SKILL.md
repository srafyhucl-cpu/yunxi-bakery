...
### 单函数职责单一，超过 50 行触发职责评审

50 行是复杂度信号，不是机械拆分目标。先判断函数是否混合 I/O、业务决策、格式转换或多个独立变化原因；只有能提取出稳定、可独立测试的职责时才拆。单一算法、协议映射或状态转换若保持内聚，可以记录理由后保留。
...
- [ ] 超过 50 行的函数已完成职责评审；若拆分，边界可独立测试且没有增加状态穿透
- [ ] 函数参数 ≤ 5 个（超出已封装数据类）
